...
from database import (
    ...
    get_examens, get_examen, add_examen, update_examen_status, delete_examen,
...
def toon_sidebar():
    ...
    with st.sidebar:
        ...
        if rol in KAN_PLANNEN:
            paginas += ["👁️ Surveillanten", "🗓️ Kalender beheer", "⬇️ Import & Export"]
...
def toon_planformulier():
    ...
    kan = res["ok"] or (override and st.session_state.rol in KAN_OVERRULEN)
    if st.button("✅ Inplannen", type="primary", disabled=not kan):
        if override and not res["ok"] and not override_reden:
        ...
        with tab:
            ...
            for e in examens:
                with st.expander(f"{'🚨 ' if e.get('is_fau') else ''}{e['naam']} — {e.get('programma','')} | {e.get('geschat_aantal',0)} st. | {e['status'].upper()}"):
                    ...
                    with c2:
                        st.write(f"**Tijdblok pref.:** {e.get('voorkeur_tijdblok','').capitalize()}")
...
def pagina_aanmelden():
    ...
    with st.form("aanmeld_form", clear_on_submit=True):
        st.subheader("Examengegevens")
        c1,c2 = st.columns(2)
        with c1:
            naam = st.text_input("Examennaam *")
            programma = st.text_input("Programma / afdeling *")
            examtype = st.selectbox("Examtype *", ["C","H","C/H","H1","H2","H3"])
            duur = st.number_input("Duur (minuten) *", min_value=30, max_value=480, value=120, step=30)
            geschat = st.number_input("Geschat aantal studenten *", min_value=1, max_value=500, value=50)
        with c2:
            is_fau = st.checkbox("🚨 FAU — Landelijk tentamen")
            tijdblok = st.selectbox("Voorkeurstijdblok",["middag","ochtend","avond"],
                                    format_func=lambda x: f"{x.capitalize()} ({TIJDBLOK_LABELS[x]})")
            loc_pref = st.selectbox("Locatievoorkeur", ["Breukelen","Amsterdam"])
            fmt = st.selectbox("Format", ["Cirrus","Papier"])
            nieuwe_studenten = st.checkbox("Veel nieuwe studenten?")

        c3,c4 = st.columns(2)
        with c3:
            voorkeur_datum = st.date_input("Voorkeursdatum (optioneel)", value=None)
            contactpersoon = st.text_input("Contactpersoon")
        with c4:
            budgetnummer = st.text_input("Budgetnummer")
            opmerkingen = st.text_area("Opmerkingen", height=80)

        if st.form_submit_button("📨 Indienen bij planner", type="primary", use_container_width=True):
            if not naam.strip() or not programma.strip():
                st.error("Naam en programma zijn verplicht.")
            else:
                max_cap = 350 if loc_pref == "Breukelen" else 85
                if geschat > max_cap:
    ...
    if up:
        try:
            ...
            if st.button("✅ Importeren", type="primary"):
                n, _ = import_examens_uit_excel(df)
...
def laad_demo_data():
    demo = [
        ("Statistical Reasoning","Accountancy","C",0,"middag",120,10,"Breukelen"),
        ("Essentials of Auditing","Accountancy","C",0,"middag",210,20,"Breukelen"),
        ("Financial Auditing, Landelijk","Accountancy","C",1,"ochtend",480,286,"Breukelen"),
        ("Auditing Beginselen","Accountancy","C",0,"middag",210,220,"Breukelen"),
        ("FAT PreMaster","Accountancy","C/H",0,"middag",120,175,"Breukelen"),
        ("FAT Master","Accountancy","C",0,"middag",120,150,"Breukelen"),
        ("Ondernemingsrecht","Accountancy","C/H",0,"middag",180,150,"Breukelen"),
        ("Belastingrecht 1","Accountancy","C/H",0,"middag",180,220,"Breukelen"),
        ("Belastingrecht 2","Accountancy","C/H",0,"middag",180,80,"Breukelen"),
        ("BIV-Inleiding","Accountancy","C/H",0,"middag",180,60,"Breukelen"),
        ("Managerial Finance","MSc34","C",0,"middag",180,66,"Amsterdam"),
        ("RBAI retake","MSc34","H",0,"ochtend",120,9,"Amsterdam"),
        ("Leadership","MBA","C",0,"avond",120,45,"Breukelen"),
        ("Corporate Governance","Accountancy","C",0,"ochtend",120,100,"Breukelen"),
        ("Fraude & Witwassen","Accountancy","C/H",0,"middag",120,150,"Breukelen"),
    ]
    for naam, prog, etype, is_fau, tb, duur, st_n, loc in demo:
        add_examen({"naam":naam,"programma":prog,"examtype":etype,"is_fau":is_fau,
                    "voorkeur_tijdblok":tb,"duur_minuten":duur,"geschat_aantal":st_n,
...
def main():
    ...
    elif pagina == "Import & Export" and rol in KAN_PLANNEN:
        pagina_export()
    else:
        pagina_kalender()
